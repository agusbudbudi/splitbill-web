...
export const ExpenseList = () => {
  ...
  return (
    <div id="expense-list-section" className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {SHOW_QUICK_ASSIGN && (
        <div className="relative -mx-4 px-3 pt-4 space-y-4 rounded-t-sm overflow-hidden">
            ...
            <Card
              ...
              className={cn(
                ...
                expense.who.length === 0 || !expense.paidBy
                  ? "border-amber-200 bg-amber-50/20 backdrop-blur-xs shadow-soft text-card-foreground"
                  : "border-primary/0 bg-white shadow-sm",
              )}
            >
                ...
                <div className="flex justify-between items-start gap-4">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      ...
                      className="p-1.5 rounded-sm bg-primary/5 text-primary hover:bg-primary hover:text-white transition-all cursor-pointer"
                    >
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => setConfirmDeleteId(expense.id)}
                      className="p-1.5 rounded-sm bg-destructive/5 text-destructive hover:bg-destructive hover:text-white transition-all cursor-pointer"
                    >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {people.map((name) => (
                        <PersonChip
                          ...
                          name={name}
                          variant="payer"
                          selected={expense.paidBy === name}
                          onClick={(clicked) =>
                            updateExpense(expense.id, {
                              paidBy: expense.paidBy === clicked ? "" : clicked,
                            })
                          }
                        />
                      ))}
