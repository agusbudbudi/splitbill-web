...
export const SavedBestiesSelection = ({
  ...
  return (
    <div
      className={cn(
        "rounded-sm border bg-white transition-all overflow-hidden",
          ...
          <div className="space-y-6 p-4 border-t border-primary/10">
            <div className="flex items-center gap-2 px-1">
              ...
              <p className="text-[11px] text-muted-foreground">
                Klik <span className="text-primary font-semibold">Circle Gua</span> atau tombol{" "}
                <span className="text-primary font-semibold">Tambah</span> buat langsung ajak mereka!
              </p>
            </div>
            ...
            {groupsWithMembers.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                  <label className="text-xs font-bold text-foreground/50 block">
                    Circle Gua 🤝
                  </label>
                </div>
                    ...
                    return (
                      <div key={group.id} className="relative shrink-0">
                        <button
                          ...
                          className={cn(
                            "flex flex-col p-3 rounded-sm border transition-all active:scale-95 w-[130px] text-left gap-2 h-full cursor-pointer",
                            allMembersAdded
            ...
            {friends.length > 0 && (
              <div className="space-y-3">
                <label className="text-xs font-bold text-foreground/50 block px-1">
                  Teman Tersimpan ✨
                </label>

                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground/50 z-10" />
                  <Input
                    type="text"
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    placeholder="Cari teman tersimpan..."
                    className="h-12 pl-9 pr-3"
                  />
                </div>

                <div className="divide-y divide-primary/5 rounded-sm border border-primary/5 overflow-hidden">
                  {displayedFriends.length === 0 ? (
                    ...
                    displayedFriends.map((friend) => {
                      ...
                      return (
                        ...
                        >
                          <img
                            src={getFriendAvatarUrl(friend.name)}
                            alt={friend.name}
