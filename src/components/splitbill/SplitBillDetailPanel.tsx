...
import { MonitorStatusButton } from "@/components/splitbill/MonitorStatusButton";
...
function SplitBillDetailContent() {
  ...
  if (error || !bill) {
    return (
      <div className="w-full max-w-[600px] mx-auto flex flex-col items-center justify-center py-20">
  ...
  return <SplitBillDetailView bill={bill} showBanner={showBanner} setShowBanner={setShowBanner} router={router} />;
}
    ...
    <div className="w-full max-w-[600px] mx-auto space-y-4">
      <button
        onClick={() =>
          canGoBackInApp()
            ? router.back()
            : router.push("/member/history?tab=split-bill")
        }
        className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground hover:text-primary transition-colors group cursor-pointer"
      >
      ...
      <div className="relative w-full flex-1 flex flex-col">
        ...
        <main className="relative z-10 flex-1 flex flex-col">
          <div className="pt-3">
            <BillSummary ref={billSummaryRef} billData={bill} isPublic={false} hideShareActions hideMonitorButton />
          </div>

          {/* Share section */}
          <div className="relative pt-4 pb-8 space-y-4 rounded-b-lg overflow-hidden">
            <div className="relative z-10 space-y-4">
              <div className="space-y-1 px-1">
                <h2 className="text-lg font-black text-foreground">
                  Bagikan Hasil Split Bill!
                </h2>
                <p className="text-xs text-muted-foreground font-medium">
                  Biar temen-temen langsung tau nominal yang harus dibayar.
                </p>
              </div>
              ...
              <MonitorStatusButton
                ...
                activityName={bill.activityName}
                settlementInstructions={settlementInstructions}
                selectedPaymentMethodIds={bill.selectedPaymentMethodIds || []}
              />
...
export function SplitBillDetailPanel() {
  return (
    <Suspense
      fallback={
